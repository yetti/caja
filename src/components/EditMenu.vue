<template>
  <div class="relative flex justify-end items-center" ref="target">
    <button
      @click="toggleMenu()"
      type="button"
      class="
...
<script setup lang="ts">
  import { useEditMenuMachine } from '~/state/edit-menu-machine'
  import { onClickOutside } from '@vueuse/core'

  const target = ref('')
  const toggleMenu = () => {
    send({ type: 'TOGGLE_EDIT_MENU' })
  }

  const closeMenu = () => {
    send({ type: 'CLOSE_EDIT_MENU' })
  }

  const { state, send } = useEditMenuMachine()
